...
{
    public partial class PDFConverter_CS : UserControl, IIKAVA_Systembehandler_Plugin
    {
        ...
        public PDFConverter_CS()
        {
            ...
            lblIPAddress.Text = Dns.GetHostEntry(Dns.GetHostName()).AddressList.FirstOrDefault(ip => ip.AddressFamily == AddressFamily.InterNetwork).ToString();
        }
        ...
        {
            get { return "PDF Converter Client/Server"; }
        }
        ...
        {
            grpServerSetup.Enabled = false;
            grpClientSetup.Enabled = true;
        }

        private void radioButton1_CheckedChanged(object sender, EventArgs e)
        {
            grpServerSetup.Enabled = true;
            grpClientSetup.Enabled = false;
            ...
            service.Register();
            GetServices();
            //service =  new Zeroconf.IService() { Name = "KDRS PDFConverter Server" };
        }

        List<Zeroconf.IZeroconfHost> responses = new List<IZeroconfHost>();
        ...
        public async Task EnumerateAllServicesFromAllHosts()
        {
            ...
            var responsearray = await ZeroconfResolver.ResolveAsync(domains.Select(g => g.Key));
            responses = responsearray.ToList<Zeroconf.IZeroconfHost>();
        }
    }
}